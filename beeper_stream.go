...
type BeeperStreamManager struct {
	...
	publishedByStreamID   map[id.StreamID]*beeperStreamPublished
	subscriptions         map[beeperStreamKey]*beeperStreamSubscription
	subscriptionsByStream map[id.StreamID]*beeperStreamSubscription
...
func (cli *Client) BeeperStreams() *BeeperStreamManager {
	...
	manager := &BeeperStreamManager{
		...
		publishedByStreamID:   make(map[id.StreamID]*beeperStreamPublished),
		subscriptions:         make(map[beeperStreamKey]*beeperStreamSubscription),
		subscriptionsByStream: make(map[id.StreamID]*beeperStreamSubscription),
		now:                   time.Now,
	}
	cli.beeperStream = manager
	cli.AddToDeviceInterceptor(manager.handleToDeviceEvent)
	return manager
}
...
func (m *BeeperStreamManager) Register(ctx context.Context, roomID id.RoomID, eventID id.EventID, descriptor *event.BeeperStreamInfo) error {
	...
	if existing := m.publishedStreams[key]; existing != nil {
		...
		if existing.descriptor != nil && existing.descriptor.Encryption != nil {
			delete(m.publishedByStreamID, existing.descriptor.Encryption.StreamID)
		}
	}
	m.publishedStreams[key] = state
	if state.descriptor.Encryption != nil {
		m.publishedByStreamID[state.descriptor.Encryption.StreamID] = state
	}
	m.lock.Unlock()
...
func (m *BeeperStreamManager) Unregister(roomID id.RoomID, eventID id.EventID) {
	...
	if state.descriptor != nil && state.descriptor.Encryption != nil {
		delete(m.publishedByStreamID, state.descriptor.Encryption.StreamID)
	}
...
func (m *BeeperStreamManager) Subscribe(ctx context.Context, roomID id.RoomID, eventID id.EventID, descriptor *event.BeeperStreamInfo) error {
	...
	if existing := m.subscriptions[key]; existing != nil {
		...
		if existing.descriptor.Encryption != nil {
			delete(m.subscriptionsByStream, existing.descriptor.Encryption.StreamID)
		}
	}
	...
	if sub.descriptor.Encryption != nil {
		m.subscriptionsByStream[sub.descriptor.Encryption.StreamID] = sub
	}
	m.lock.Unlock()
...
func (m *BeeperStreamManager) Unsubscribe(roomID id.RoomID, eventID id.EventID) {
	...
	if sub != nil {
		delete(m.subscriptions, key)
		if sub.descriptor.Encryption != nil {
			delete(m.subscriptionsByStream, sub.descriptor.Encryption.StreamID)
		}
	}
	m.lock.Unlock()
	if sub != nil {
...
func (m *BeeperStreamManager) handleToDeviceEvent(ctx context.Context, evt *event.Event) bool {
	...
	case event.ToDeviceBeeperStreamUpdate:
		return m.handlePlainUpdateEvent(ctx, evt)
...
func (m *BeeperStreamManager) handlePlainUpdateEvent(ctx context.Context, evt *event.Event) bool {
...
func (m *BeeperStreamManager) handleEncryptedEvent(ctx context.Context, evt *event.Event) bool {
	...
	if content == nil || content.Algorithm != id.AlgorithmBeeperStreamAESGCM {
		return false
	} else if m.isForDifferentUser(evt) {
		return true
	...
	m.lock.RLock()
	published := m.publishedByStreamID[content.StreamID]
	sub := m.subscriptionsByStream[content.StreamID]
	m.lock.RUnlock()

	if published != nil {
		m.tryHandleEncryptedSubscribe(ctx, evt, content, published)
		return true
	}
	if sub == nil {
		return true
	}
	return m.handleEncryptedUpdate(ctx, evt, content, sub)
}

func (m *BeeperStreamManager) tryHandleEncryptedSubscribe(ctx context.Context, evt *event.Event, content *event.EncryptedEventContent, state *beeperStreamPublished) bool {
	if state.gcm == nil {
		return false
	}
	payload, err := decryptPayload(content, state.gcm)
	if err != nil {
		zerolog.Ctx(ctx).Debug().Err(err).Msg("Failed to decrypt custom encrypted subscribe")
		return false
	}
...
func (m *BeeperStreamManager) handleEncryptedUpdate(ctx context.Context, evt *event.Event, content *event.EncryptedEventContent, sub *beeperStreamSubscription) bool {
	logicalType, parsedContent, err := decryptEvent(content, sub.descriptor.Encryption.Key)
	if err != nil {
		m.log.Debug().Err(err).
	...
	evt.Type = event.ToDeviceBeeperStreamUpdate
...
func (m *BeeperStreamManager) sendUpdate(ctx context.Context, descriptor *event.BeeperStreamInfo, gcm cipher.AEAD, update *event.Content, subscribers []beeperStreamSubscriber) error {
	...
	if descriptor != nil && descriptor.Encryption != nil && gcm != nil {
		encrypted, encErr := encryptPayload(eventType, update, descriptor.Encryption.StreamID, gcm)
		if encErr != nil {
			return encErr
		}
		eventType = event.ToDeviceEncrypted
...
func (m *BeeperStreamManager) tryPendingSubscribe(ctx context.Context, evt *event.Event) bool {
	...
	case event.ToDeviceEncrypted:
		...
		m.lock.RLock()
		state := m.publishedByStreamID[content.StreamID]
		m.lock.RUnlock()
		if state == nil {
			return false
		}
		return m.tryHandleEncryptedSubscribe(ctx, evt, content, state)
	default:
		return false
	}
}
...
func descriptorEqual(a, b *event.BeeperStreamInfo) bool {
	...
	default:
		return a.Encryption.Algorithm == b.Encryption.Algorithm &&
			...
			a.Encryption.StreamID == b.Encryption.StreamID
	}
}
