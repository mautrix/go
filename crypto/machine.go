...
func (mach *OlmMachine) storeGroupSession(ctx context.Context, igs *InboundGroupSession) error {
	...
	zerolog.Ctx(ctx).Debug().
		...
		Str("session_id", igs.ID().String()).
		Str("sender_key", igs.SenderKey.String()).
		Int64("max_age_ms", igs.MaxAge).
		Int("max_messages", igs.MaxMessages).
		Bool("is_scheduled", igs.IsScheduled).
		Uint32("first_message_index", igs.Internal.FirstKnownIndex()).
		Msg("Stored inbound group session")
