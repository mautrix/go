...
type VerificationTransaction struct {
	...
	TheirDeviceID id.DeviceID `json:"their_device_id"`
	// TheirUserID is the user ID of the other user.
	TheirUserID id.UserID `json:"their_user_id"`
	// TheirSupportedMethods is a list of verification methods that the other
	// device supports.
	TheirSupportedMethods []event.VerificationMethod `json:"their_supported_methods"`
	...
	SentToDeviceIDs []id.DeviceID `json:"sent_to_device_ids"`

	// QRCodeSharedSecret is the shared secret that was encoded in the QR code
	// that we showed.
	QRCodeSharedSecret []byte `json:"qr_code_shared_secret"`

	StartedByUs              bool                                 `json:"started_by_us"`               // Whether the verification was started by us
	StartEventContent        *event.VerificationStartEventContent `json:"start_event_content"`         // The m.key.verification.start event content
	Commitment               []byte                               `json:"committment"`                 // The commitment from the m.key.verification.accept event
	MACMethod                event.MACMethod                      `json:"mac_method"`                  // The method used to calculate the MAC
	EphemeralKey             *ECDHPrivateKey                      `json:"ephemeral_key"`               // The ephemeral key
	EphemeralPublicKeyShared bool                                 `json:"ephemeral_public_key_shared"` // Whether this device's ephemeral public key has been shared
	OtherPublicKey           *ECDHPublicKey                       `json:"other_public_key"`            // The other device's ephemeral public key
	ReceivedTheirMAC         bool                                 `json:"received_their_mac"`          // Whether we have received their MAC
	SentOurMAC               bool                                 `json:"sent_our_mac"`                // Whether we have sent our MAC
	ReceivedTheirDone        bool                                 `json:"received_their_done"`         // Whether we have received their done event
	SentOurDone              bool                                 `json:"sent_our_done"`               // Whether we have sent our done event
}
