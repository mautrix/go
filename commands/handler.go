...
type Handler[MetaType any] struct {
        ...
        //Param are parameters for each command.
        Params []*event.MSC4391.Parameter

        //This is the command syntax itself
        BCommand []*event.MSC4391.BotCommand

        //This is for input
        Input []*event.MSC4332.BotCommandInput
...
func HandleCommands(cmd *Handler[MetaType])(prefix string) command(){
        if(Input.Syntax == Command){
                return &Handler[MetaType].ce.Command
        } else {
			MakeUnknownCommandHandler(Input.Syntax)
		}
}
